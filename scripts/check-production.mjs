...
try {
  for (const backend of ['auto', 'webgl2']) {
    ...
    for (const [i, letter] of [...'RAU'].entries()) await page.getByRole('combobox', { name: `Inicial ${i + 1}` }).selectOption(letter);
    await page.getByRole('button', { name: /VER RÉCORDS/ }).click();
    await page.getByRole('dialog', { name: /Récords de pilotos/ }).waitFor({ state: 'visible' });
    ...
    await page.getByRole('button', { name: 'VER RÉCORDS' }).click();
    await page.keyboard.press('Escape');
    await page.getByRole('dialog', { name: /Récords de pilotos/ }).waitFor({ state: 'hidden' });
    await page.getByRole('dialog', { name: /Vuelo en pausa/ }).waitFor({ state: 'visible' });
    ...
    for (const [i, letter] of [...'RAU'].entries()) assert.equal(await page.getByRole('combobox', { name: `Inicial ${i + 1}` }).inputValue(), letter);
