...
export function spawnEnemy(ctx: CombatContext, kind: EnemyKind, x: number, y: number, telegraph = 0.65): Enemy {
  ...
  const enemy: Enemy = { id: ctx.nextId(), kind, x: wrapX(x, CONFIG.worldWidth), y, vx: 0, vy: 0,
    hp: COMBAT.enemyHp[kind], hitsRemaining: profile.enemyHitsToKill, target: null, phase: kind === 'harvester' ? 'seek' : 'hunt',
      ...
      if (c) {
        if (e.phase === 'lift') {
          ...
          if (e.y >= COMBAT.abductionY) {
            ...
            e.hp = COMBAT.enemyHp.wraith; e.hitsRemaining = profile.enemyHitsToKill; e.phase = 'hunt'; ctx.emit('mutation', e.x, e.y);
          }
        } else {
