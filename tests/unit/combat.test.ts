...
import { damageEnemy, hitPlayer, updateProjectiles } from '../../src/game/combat/CombatSystem';
...
describe('combate y oleada', () => {
  ...
  it('cada dificultad exige su número exacto de impactos para toda familia enemiga', () => {
    const kinds = Object.keys(COMBAT.enemyHp) as EnemyKind[];
    for (const [difficulty, impacts] of [['cadet', 1], ['relaxed', 1], ['normal', 2], ['hard', 2], ['expert', 3], ['overdrive', 4]] as const) {
      for (const kind of kinds) {
        const s = new Simulation(8042, 'combat-basic', difficulty);
        s.state.colonists = [];
        const enemy = spawnEnemy(s.context, kind, s.state.player.x + 8, s.state.player.y, 0);
        for (let hit = 1; hit <= impacts; hit++) {
          s.state.shots.push({ id: 99010 + hit, x: enemy.x, y: enemy.y, vx: 0, vy: 0, remaining: 1, team: 'player' });
          updateProjectiles(s.context, 0, { centerX: s.state.player.x, width: 200 });
          expect(s.state.enemies.some(current => current.id === enemy.id)).toBe(hit < impacts);
        }
